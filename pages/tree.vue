<template>
    ...
    <div class="tree h-screen overflow-hidden" :key="reset">
        <div class="pb-40 h-container v-container tree-wrap h-full overflow-hidden">
            ...
            <div class="timeline-container">
                <transition name="page">
                    <timeline :start="dateRange[0]" :end="dateRange[1]" @rangeChange="scale" ref="timeline" :startRange="activeDateRange" v-if="tree==1 && !tree2" :key="tmk"></timeline>
                </transition>
            </div>

            <!-- Tree -->
            <div class="tree-container w-4/5 lg:w-11/12 mx-auto h-full rounded-lg relative" ref="container">
                <img class="progeny-description" src="@/assets/img/tree-progeny.png">
                <transition name="page">
                    <div class="selected absolute rounded-lg p-4 shadow-md m-4" v-if="selected">
                        ...
                        <h3 class="ar-font" v-if="selected.nameAr">{{selected.nameAr}}</h3>                        
                    </div>
                </transition>
                ...
                <div :class="[ 'g-cursor', { 'g-cursor_hover': hover }, {'g-cursor_hide': hideCursor} ]">
                    ...
                    <div class="cursor-line" :style="cursorLine" v-if="tree==1 && !tree2">
                ...
                <!-- Back btn -->
                <h4 class="text-theme font-semibold opacity-0 l" @click="back" ref="backTxt" v-if="tree2">
                    {{$t('back')}}</h4>

                <!-- Lang switch -->
                <transition name="page">
                    <h3 class="text-theme font-semibold text-3xl absolute right-0 pr-8 opacity-25 l" v-if="tree2">
                        {{$i18n.locale=='en' ? tribeName.en : tribeName.ar}}</h3>
                </transition>

                <!-- Main svg - modified by d3: -->
                <svg width="100%" height="100%" ref="svg1" id="tree1" :class="`s-${this.yM >= 1 || tree2 ? 1: 0}`">

                    <!-- Main tree el: -->
                    <g class="tree-g" :transform="`translate(${treeOffset})`" ref="treeGrp" :class="anim?'anim':''">
                        <g class="links"></g>
                        <g class="nodes"></g>
                    </g>
                    ...
                    <!-- Tribe tree el: -->
                    <g class="tree2-g"
                       :transform="`translate(${tree2Offset[0]}, ${tree2Offset[1]}) scale(${this.tree2Scale})`"
                       ref="tree2Grp" :class="tree2?'active':''">
            ...
            <transition name="pages">
                ...
                <div class="tribe-select text-center cursor-pointer" v-if="!tree2">
                    <a v-for="(tribe, index) in tribes" :key="index" class="text-theme italic mx-2"
                       @click="loadTribe(tribe)">{{tribe.nameEn}}</a>
                </div>
            </transition>
        </div>
        ...
        <transition name="page">
            <!-- Biography modal: -->
            <div class="modalContainer w-screen h-screen fixed flex content-center justify-center top-0 left-0 p-16"
                 v-if="modal">
                <div class="w-screen h-screen fixed top-0 left-0 bg" :class="modal?'active':''">&nbsp;</div>
                <div class="modal m-auto rounded-lg p-12 flex flex-col w-full h-full" ref="modal">
                    
                    <div class="w-full h-full flex flex-col" ref="modalContent">
                        <svg xmlns="http://www.w3.org/2000/svg" width="11.697" height="11.697"
                             viewBox="0 0 11.697 11.697" @click="closeModal"
                             class="absolute close cursor-pointer right-0">
                            <g id="close" transform="translate(-7675.651 -37.651)">
                                <line id="Line_12" data-name="Line 12" x2="10" y2="10"
                                      transform="translate(7676.5 38.5)" fill="none" stroke="#057711"
                                      stroke-width="2.4"/>
                                <line id="Line_13" data-name="Line 13" y1="10" x2="10"
                                      transform="translate(7676.5 38.5)" fill="none" stroke="#057711"
                                      stroke-width="2.4"/>
                            </g>
                        </svg>
...
<script>
    ...
    export default {
        ...
        async asyncData({$axios}) {
            ...
            return {tribes: data.tribes, tree1: tdata.tree, dateRange: dr, activeDateRange: dr}
        },
        mounted() {
            //Tree
            if (this.$route.query.tribe) {
               ...
               this.initialRoad = true;
                ...
                setTimeout(() => {
                    ...
                    function getRandomInt(min, max) { //Randomise node positions
                        min = Math.ceil(min);
                        max = Math.floor(max);
                    ...
                    //Fade nodes in
                    this.$GSAP.TweenMax.to('.tree-g .node .t1, .tree-g .links, .tree-g .logoImg', 0.5, {
                        css: {opacity: 1},
                        delay: 1.5,
                        onComplete: () => {
                            this.anim = false
        ...
        computed: {
            ...
            cursorLine() {
                return `transform: translateY(${this.yParent - 140}px) translateZ(0) translate3d(0, 0, 0);`
        ...
        methods: {
            ...
            openModal(node) {
                this.$axios.$post('/api', {call: 'getBio', payload: {nodeId: node.id}}).then((bio) => {
                    if (bio) {
                        this.modalNode = node
                        this.bio = bio
            ...
            calcOffset() {
                // svg positioning
                this.treeOffset = -22 + (this.$refs.svg1.clientWidth / 2) - (this.$refs.treeGrp.getBoundingClientRect().width / 2)
                this.tree2Offset = [(this.$refs.svg1.clientWidth / 2) - (this.$refs.tree2Grp.getBoundingClientRect().width / 2), (this.$refs.svg1.clientHeight / 2) - (this.$refs.tree2Grp.getBoundingClientRect().width / 2)]
            },
            updateTree1() {
                //Various code copied from initTree1() - called every time the tree needs to be updated
                var yM = (this.$refs.timeline.$el.getBoundingClientRect().height / (this.activeDateRange[1] - this.activeDateRange[0]));
                ...
                var node = d3.select('#tree1 g.nodes')
                    .selectAll('g.node')
                    // .attr('transform', (d)=>{return `translate(${calcX(d.x)},${calcY(d.data.year)}) ${(!d.parent)?'scale(1.4)':''}`})
                    .attr('transform', (d) => {
                        return `translate(${calcX(d.x)},${calcY(d.data.year)}) ${(!d.parent) ? 'scale(1.4)' : ''}`
                    })
                var links = d3.select('#tree1 g.links')
                    .selectAll('path.link')
                    .attr('d', d => d.horizontal ? horizontal(d) : link(d))

                d3.select('#tree1 g.links').selectAll('.marriage-link').remove();
                ...
                targetArray.forEach((item, idx) => {
                    if (item.data.wife_id) {
                        ...
                        const wife_node = targetArray.filter((link) => link.data.id === wife_id)
                ...
                manageLinkIds.forEach((link) => {
                    const marriage_link = d3.linkHorizontal()({
                        source: [calcX(linkData[link.source].target.x), calcY(linkData[link.source].target.data.year)],
                        target: [calcX(linkData[link.target].target.x), calcY(linkData[link.target].target.data.year)]
                    });
                    ...
                    .each(function() {
                        ...
                        d3.selectAll("#tree1 g.name")
                            ...
                            if(this != that) {
                                ...
                                // var translate2 = [-dx, - dy];
                                d3.select(this).attr("transform", "translate(" + translate2 + ")");
                                d3.select(that).attr("transform", "translate(" + translate1 + ")");
                                a = this.getBoundingClientRect();
                                }
                            }
                        });
            ...
            redraw (treeLayout) {
                ...
                var calcY = (year) => {
                    return (year - this.activeDateRange[0]) * (this.$refs.timeline.$el.getBoundingClientRect().height / (this.activeDateRange[1] - this.activeDateRange[0]))
                }
                ...
                var nodes = d3.select('#tree1 g.nodes')
                    .selectAll('g.node')
                ...
                var node = nodes.append('g')
                    ...
                    .attr('transform', (d) => {                        
                        ...
                        d.data.x0 = d.x
                        return `translate(${calcX(d.data.x0)},${calcY(d.data.year)}) ${(!d.parent) ? 'scale(1.4)' : ''}`
                ...
                nodeR.on("click", (d) => {
                        ...
                        var nodeExit = d3.select('#tree1 g.nodes')
                                .selectAll('g.node')
                                .remove()
                        d3.select('#tree1 g.links').selectAll('*').remove();

                        this.redraw(treeLayout)
                    })

                // Node name txt:
                var name = node.append('g')
                    .classed('name', true)
                    .attr('transform', (d) => {
                        if (!!d.data.x0) {
                            d.x = d.data.x0
            ...
            initTree2() {
                ...
                var back = tree(d3.hierarchy({royal: true, id: -1, nameEn: 'Back to main tree'}))
                ...
                var createBackLink = (link) => {
                    links.data(link)
                        .enter().append('path').attr("stroke", "#057711")
                        .attr("d", d3.linkRadial()
                            .angle(d => Math.PI) //Straight
                            .radius(d => -d.y));
                }

                const links = svgGrp.append("g")
                    .classed('links', true)
                    .attr("fill", "none")
                ...
                links.data(root.links().filter((l) => {
                    ...
                    .enter().append("path").attr("stroke", "#666")
                    .attr("d", d3.linkRadial()
                ...
                this.$GSAP.TweenMax.to('.tree2-g *', 0.5, {
                    css: {opacity: 0}, onComplete: () => {
                        ...
                        setTimeout(() => {
                            ...
                            this.tmk++
                            this.selected = false
                            this.$GSAP.TweenMax.to('.tree-g, .tree-g .links', 0.5, {css: {opacity: 1}})
                            this.$GSAP.TweenMax.to(this.$refs.backTxt, 0.5, {css: {opacity: 0}})
                        }, 50);
                    }
                })
            },
            moveCursor(e) {
                ...
                if (this.tree == 1) {
                    var tMinY = this.$refs.timeline.$el.offsetParent.offsetTop + 20,
                        ...
                        tMaxY = tMinY + tHeight,
                        date = Math.round(((this.yChild - tMinY) * (this.activeDateRange[1] - this.activeDateRange[0]) / tHeight) + this.activeDateRange[0]);
            ...
            pan(e) {
                if (this.tree2 && this.dragging) {
                    ...
                    var x = 122 + (this.$refs.svg1.clientWidth / 2) - (this.$refs.treeGrp.getBoundingClientRect().width / 2);
            ...
            loadTribe(t) {
                ...
                this.$axios.$post('/api', {call: 'getTribeTree', payload: {_id: id}}).then((res) => {
                    this.tree2 = res
                    this.initTree2()
                    setTimeout(() => {
                        // Animating to tribe tree:
                        var nonMatches = [],
                            tree1Nodes = Array.from(document.querySelectorAll(".tree-g [data-id]")),
                            tree2Nodes = Array.from(document.querySelectorAll(".tree2-g [data-id]")),
                            tree2NodesIds = tree2Nodes.map(val => val.getAttribute('data-id')),
                            ...
                            animTargets = matches.map(val => [val, tree2Nodes.find(tVal => val.getAttribute('data-id') == tVal.getAttribute('data-id'))])

                        //fade links + non matches
                        this.$GSAP.TweenMax.to([nonMatches, '.tree-g .links', '.tree-g .name', '.tree-g .logoImg'], 0.6, {css: {opacity: 0}})
                        var offsetx = this.treeOffset + this.$refs.svg1.getBoundingClientRect().left + 35;
                        ...
                        animTargets.forEach((x, index, array) => {
                            ...
                            this.$GSAP.TweenMax.to(x[0], 1, {
                                css: {
                                    x: (x[1].getBoundingClientRect().width / 2) + x[1].getBoundingClientRect().left - offsetx,
                                    y: (x[1].getBoundingClientRect().height / 2) + x[1].getBoundingClientRect().top - offsety
                                }, delay: 0.4, ease: this.$GSAP.Power2.in
                            })

                            if (index == array.length - 1) {
                                // fade in elements at various times and durations
                                this.$GSAP.TweenMax.to('.tree2-g .circle', 0.5, {css: {opacity: 1}, delay: 0.8})
                                this.$GSAP.TweenMax.to(matches, 0.2, {
                                    css: {opacity: 0}, delay: 1.2, onComplete: () => {
                                        d3.selectAll(".tree-g .links > *, .tree-g .nodes > *").remove()
                                        this.$GSAP.TweenMax.set('.tree-g', {css: {opacity: 0}})
                                    }
                                })
                                this.$GSAP.TweenMax.to('.tree2-g .nodes', 0.2, {css: {opacity: 1}, delay: 1.2})
                                this.$GSAP.TweenMax.to('.tree2-g .links', 0.3, {css: {opacity: 1}, delay: 1})
                                this.$GSAP.TweenMax.to(this.$refs.backTxt, 0.3, {css: {opacity: 1}, delay: 1})
                            }
                        })
                    }, 10);
                })
...
<style lang="scss">
    ...
    .tree {
        ...
        background: url('../assets/img/watermark-min.png'), url('../assets/img/bg2-min.jpg');
